...
# inputfile = r'C:\Users\qinci\Desktop\a\001.log'   #仅仅用作debug测试
...
def mysplit(outputPath,inputfile,pattern,Cli):
    ...
    for i in range(len(l1)):
        path0 =re.sub(':','',re.findall(Cli,l1[i])[0][:-1])  #只取第一个元素，并且去除元素后的换行符,且去除部分show 命令后的‘：’
...
def main():
    ...
    pattern = pCisco
    try:
        opts,args = getopt.getopt(sys.argv[1:],'i:ch')
    ...
    mysplit(mypath_name(inputfile),inputfile,pattern,Cli)
    ####当前，仅cisco 设备调试正常
